...
export default ({  navigate }) => {
    ...
    const { contacts ,searchTerm} = useSelector(state => state)
    useEffect(() => {
    ...
    const ungroupContacts = {
        title: '#',
        data: []
    }
    const arrayOfInitials = groupList.map(group => group.title.toLowerCase())
    ...
    const renderItem = ({ item, index, section, ...rest }) => (
        <Pressable onPress={() => {
            ...
            navigate(CONTACT_DETAILS, { contact: item })
        }}>
            {
